...
#define MAX_VOLTAGE       3.45 //3.3V voltage regulator reading from your multimeter (will probably be between 3.3-3.4V)
...
float check_max(float val, float max_val){
  if (val > max_val){
     return val;
  } else {
    return max_val;
...
float check_min(float val, float min_val) {
